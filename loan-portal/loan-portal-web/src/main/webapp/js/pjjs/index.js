$(function(){
	...
	$('.news-main>div').hide();
	utils.ajax({
        url:'http://120.76.203.19:8090/shzc_test/WEB-PC/app/index.do',
        dataType:'json',
...
//        		$('#userTotal').text(data.totalUserMap.userTotal);
				...
				//
        		$('#ty-rate').text(data.mapList6[0].annualRate.toFixed(2));
        		$('#ty-date').text(data.mapList6[0].deadline+'天');
        		
        		$('#ty-btn').click(function(){
        			window.location.href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id="+data.mapList6[0].id;
        		});
        		//理财新手标
//        		if (data.isNewInvest == 1) {
//        			$("#xs-btn").attr("disabled",true);
//        			$("#xs-btn").css("background-color", "#b0b0b0");
//        		}
        		//
//        		$('#xs-rate').text(data.mapList5[0].annualRate.toFixed(2));
//        		$('#xs-date').text(data.mapList5[0].deadline+'月');
//        		$('#xs-btn').click(function(){
//        			window.location.href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id="+data.mapList5[0].id;
//        		});
        		//恒金保
        		var h = $('#product-h');
        		...
        		if(hList.length>0){
        			for(var i = 0;i<hList.length;i++){
    					...
    					'<p class="product-title"><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id='+hList[i].id+'">'+hList[i].borrowTitle+canZr+'</a></p>'+
    					'<p class="p-rate"><span>'+hList[i].annualRate+'</span><span class="small">%</span></p>'+
    				...
    				'</div><div class="bottom"><div class="line icon icon-progress">'+
    					...
    					'<p class="icon icon-time">投资期限：'+hList[i].deadline+'个月</p></div>'+
    				'<div class="submit">'+
        		...
        		if(pList.length>0){
        			for(var i = 0;i<pList.length;i++){
    					...
    					'<p class="product-title"><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id='+pList[i].id+'">'+pList[i].borrowTitle+canZr+'</a></p>'+
    					'<p class="p-rate"><span>'+pList[i].annualRate+'</span><span class="small">%</span></p>'+
    					'<span class="p-rate-text">预期年化收益率</span>'+
    				'</div><div class="bottom"><div class="line icon icon-progress">'+
    						'<p>募集进度：</p>'+
    					...
    					'<p class="icon icon-time">投资期限：'+pList[i].deadline+'个月</p></div>'+
    				'<div class="submit">'+
        		...
        		if(dList.length>0){
        			for(var i = 0;i<dList.length;i++){
    					...
    					'<p class="product-title"><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id='+dList[i].id+'">'+dList[i].borrowTitle+canZr+'</a></p>'+
    					'<p class="p-rate"><span>'+dList[i].annualRate+'</span><span class="small">%</span></p>'+
    					'<span class="p-rate-text">预期年化收益率</span>'+
    				'</div><div class="bottom"><div class="line icon icon-progress">'+
    						'<p>募集进度：</p>'+
    					...
    					'<p class="icon icon-time">投资期限：'+dList[i].deadline+'个月</p></div>'+
    				'<div class="submit">'+
        		...
        		for(var i = 0;i<4;i++){
        			var newsLi = $('<li><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+newsList[i].id+'&flag=1" target="_blank">'+newsList[i].title+'</a><span>'+newsList[i].publishTime.substring(0,10)+'</span></li>');
        			var topNews = $('<li><span>'+newsList[i].announcementType+'&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+newsList[i].id+'&flag=1" target="_blank">'+newsList[i].title+'</a></li>');
        		...
        		for(var i=0;i<meikuList.length;i++){
        			var mikuLi = 
        				$('<li><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+meikuList[i].id+'" target="_blank" class="news-main-content-left">'+
    							'<img src="'+meikuList[i].imgPath+'" alt="'+meikuList[i].title+'" width="210" height="140"></a>'+
    						'<a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+meikuList[i].id+'" target="_blank" class="list-title">'+meikuList[i].title+'</a>'+
    						'<a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+meikuList[i].id+'" target="_blank" class="list-main">'+strChina(meikuList[i].content)+'</a></li>')
        			mikuLi.appendTo($('#dynamic .news-main-list'));
        		};
        		//行业新闻
        		var industryNews = data.meikuList;
    			for(var i = 0;i<industryNews.length;i++){
        			var partLi = $('<li><a href="http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/news.html?id='+industryNews[i].id+'" target="_blank">'+industryNews[i].title+'</a><span>'+industryNews[i].publishTime.substring(0,10)+'</span></li>');
        		...
        		for(var i=0;i<linkList.length;i++){
        			var linkLi = $('<li><a href="'+linkList[i].companyURL+'" title="'+linkList[i].companyName+'"><img src="../'+linkList[i].companyImg+'" alt="'+linkList[i].companyName+'"></a></li>');
...
function toInvest(id,status){
	if(status!=2){return};
	window.location.href = 'http://120.76.203.19:8090/shzc_test/WEB-PC/scripts/invest.html?id='+id;
}
